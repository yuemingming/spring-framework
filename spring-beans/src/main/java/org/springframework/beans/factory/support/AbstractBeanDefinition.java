...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑改该bean，及他不会被考虑作为其他bean自动装配的候选者，
	 * 但是该bean自身还是可以使用自动装配注入其他bean的
	 * 对应bean属性autowire-candidate
	 */
	private boolean autowireCandidate = true;
	/**
	 * 自动装配时出现多个bean候选者时，将作为候选者，对应bean属性primary。
	 */
	...
	/**
	 * 是否是用户定义，而不是应用程序本身定义的，创建AOP的时候为true，程序设置
	 */
	private boolean synthetic = false;
	/**
	 * 定义这个bean的应用，APPLICATION：用户，INFRASTRUCTRUE：完全内部使用，与用户无关，SUPPORT：某些复杂配置的一部分
	 * 程序设置
	 */
